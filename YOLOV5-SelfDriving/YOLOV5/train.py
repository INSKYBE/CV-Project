...
def train(hyp, opt, device, tb_writer=None):
    logger.info(f'Hyperparameters {hyp}')  # 训练参数结果保存到日志中
    log_dir = Path(tb_writer.log_dir) if tb_writer else Path(opt.logdir) / 'evolve'  # logging directory
    wdir = log_dir / 'weights'  # weights directory
    os.makedirs(wdir, exist_ok=True)  # 保存路径
    last = wdir / 'last.pt'  # 最后一次模型
    best = wdir / 'best.pt'  # 所有epoch中综合分最高的模型
    ...
    # 优化器
    nbs = 64  # nominal batch size nbs/batchsize次更新权重参数
    accumulate = max(round(nbs / total_batch_size), 1)  # accumulate loss before optimizing
    hyp['weight_decay'] *= total_batch_size * accumulate / nbs  # scale weight_decay

    pg0, pg1, pg2 = [], [], []  # optimizer parameter groups 权重，偏置，其他参数
    ...
    mlc = np.concatenate(dataset.labels, 0)[:, 0].max()  # 判断类别数是否正常
    ...
    for epoch in range(start_epoch, epochs):  # epoch
        ...
        mloss = torch.zeros(4, device=device)  # 初始化损失函数
        if rank != -1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='yolov5s.pt', help='initial weights path')  # 加载预训练模型权重参数
    parser.add_argument('--cfg', type=str, default='', help='model.yaml path')  # 网络配置
    parser.add_argument('--data', type=str, default='../DataSet/data.yaml', help='data.yaml path')  # 训练数据
    parser.add_argument('--hyp', type=str, default='data/hyp.scratch.yaml', help='hyperparameters path')  # 超参数
    parser.add_argument('--epochs', type=int, default=120)  # epoch
    parser.add_argument('--batch-size', type=int, default=120, help='total batch size for all GPUs')  # batch size
    parser.add_argument('--img-size', nargs='+', type=int, default=[640, 640],
                        help='[train, test] image sizes')  # 图像大小默认调整为640×640
    parser.add_argument('--rect', action='store_true', help='rectangular training')  # 矩形训练
    parser.add_argument('--resume', nargs='?', const=True, default=False,
                        help='resume most recent training')  # 是否接着之前的训练
    ...
    parser.add_argument('--multi-scale', action='store_true', help='vary img-size +/- 50%%')  # 是否进行多尺度训练，但是要能被32整除
    ...
    parser.add_argument('--local_rank', type=int, default=-1, help='DDP parameter, do not modify')  # 多卡时指定的GPU ID
    parser.add_argument('--logdir', type=str, default='runs/', help='logging directory')  # 日志文件
